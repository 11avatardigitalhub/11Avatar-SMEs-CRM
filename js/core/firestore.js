...
const CRM_Firestore = (function() {
    ...
    async function init() {
        try {
            ...
            if (_isOnline && _offlineQueue.length > 0) await processOfflineQueue();
            return true;
        } catch (e) {
            _available = false;
            return false;
    ...
    function _getTenantId() {
        try {
            if (window.CRM_Tenant && window.CRM_Tenant.getTenantId) return window.CRM_Tenant.getTenantId();
            if (window.CRM_Auth && window.CRM_Auth.getTenantId) return window.CRM_Auth.getTenantId();
            if (_configReady()) {
                var cached = localStorage.getItem(window.CRM_Config.app.storageKeys.TENANT_DATA);
                if (cached) {
                    try { var t = JSON.parse(cached); return t.id || null; } catch (e) { return null; }
                }
            }
            return null;
        } catch (e) { return null; }
    ...
    function _addTenantContext(data) {
        ...
        var result = {};
        for (var key in data) { if (data.hasOwnProperty(key)) result[key] = data[key]; }
    ...
    function _cacheKey(collection, docId, query) {
        ...
        if (query) {
            try { key += '_' + JSON.stringify(query); } catch (e) {}
        }
        return key;
    }
    ...
    function invalidateCache(collection) {
        ...
        var keysToDelete = [];
        _cache.forEach(function(val, key) { if (key.indexOf(prefix) === 0) keysToDelete.push(key); });
    ...
    async function getDocument(collection, docId, options) {
        options = options || {};
        try {
            if (!_available) return _getFromCache(_cacheKey(collection, docId));
            ...
            if (data.tenantId && data.tenantId !== _getTenantId()) return null;
            if (!options.includeDeleted && data._deleted) return null;
            _setCache(_cacheKey(collection, docId), data);
            return data;
        } catch (e) { return _getFromCache(_cacheKey(collection, docId)); }
    }
    ...
    async function createDocument(collection, data) {
        try {
            ...
            var now = new Date().toISOString();
            var finalData = {};
            for (var key in docData) { if (docData.hasOwnProperty(key)) finalData[key] = docData[key]; }
            ...
            if (_isOnline && _available) {
                ...
                _addToOfflineQueue({ type: 'create', collection: collection, docId: docRef.id, data: finalData });
                _setCache(_cacheKey(collection, docRef.id), finalData);
                return finalData;
            }
        } catch (e) { throw e; }
    ...
    async function setDocument(collection, docId, data) {
        try {
            ...
            if (_isOnline && _available) {
                ...
                _addToOfflineQueue({ type: 'set', collection: collection, docId: docId, data: finalData });
                _setCache(_cacheKey(collection, docId), finalData);
                return finalData;
            }
        } catch (e) { throw e; }
        ...
        try {
            ...
            if (_isOnline && _available) {
                ...
                if (cached) {
                    ...
                    for (var uk in updateData) { if (updateData.hasOwnProperty(uk)) merged[uk] = updateData[uk]; }
                ...
                _addToOfflineQueue({ type: 'update', collection: collection, docId: docId, data: updateData });
                var c2 = _getFromCache(_cacheKey(collection, docId));
                var m2 = {};
                if (c2) { for (var ck in c2) { if (c2.hasOwnProperty(ck)) m2[ck] = c2[ck]; } }
                for (var uk3 in updateData) { if (updateData.hasOwnProperty(uk3)) m2[uk3] = updateData[uk3]; }
                m2.id = docId;
        ...
        try {
            if (hardDelete) {
                if (_isOnline && _available) {
                    ...
                    _addToOfflineQueue({ type: 'hardDelete', collection: collection, docId: docId });
                }
            } else {
                await updateDocument(collection, docId, {
                    _deleted: true, _deletedAt: new Date().toISOString(), _deletedBy: _getCurrentUid()
                });
            }
            return true;
    ...
    async function restoreDocument(collection, docId) {
        return await updateDocument(collection, docId, {
            _deleted: false, _deletedAt: null, _deletedBy: null,
            _restoredAt: new Date().toISOString(), _restoredBy: _getCurrentUid()
        });
    }
    ...
    async function queryDocuments(collection, options) {
        ...
        try {
            ...
            var cacheKey = _cacheKey(collection, null, { f: filters, o: orderBy, d: orderDir, l: limit });
            ...
            if (!includeDeleted) query = query.where('_deleted', '==', false);
            ...
            return result;
        } catch (e) { return { data: [], hasMore: false, lastDoc: null, total: 0 }; }
    }
    ...
    async function getAllDocuments(collection, options) {
        try {
            ...
            while (hasMore) {
                var opts = {};
                if (options) { for (var key in options) { if (options.hasOwnProperty(key)) opts[key] = options[key]; } }
    ...
    async function commitBatch() {
        try {
            ...
            clearAllCache();
            return true;
        } catch (e) { _pendingBatch = null; return false; }
    }
    ...
    function onDocumentSnapshot(collection, docId, callback) {
        try {
            ...
            if (_activeListeners.has(listenerId)) {
                try { _activeListeners.get(listenerId)(); } catch (e) {}
            }
    ...
    function _addToOfflineQueue(operation) {
        if (_offlineQueue.length >= MAX_OFFLINE_QUEUE) _offlineQueue.shift();
        _offlineQueue.push({
            type: operation.type, collection: operation.collection,
            docId: operation.docId, data: operation.data,
            queuedAt: new Date().toISOString(), retryCount: 0
        });
        _saveOfflineQueue();
    }
        ...
        try {
            if (_configReady()) {
                ...
                if (saved) {
                    ...
                    for (var i = 0; i < parsed.length; i++) { _offlineQueue.push(parsed[i]); }
                }
            }
        } catch (e) {}
    }
    ...
    if (document.readyState === 'loading') {
        ...
        setTimeout(function() { init().catch(function() {}); }, 100);
    }

    return {
        init: init, isAvailable: isAvailable, isOnline: isOnline, getDb: getDb,
        getDocument: getDocument, createDocument: createDocument, setDocument: setDocument,
        updateDocument: updateDocument, deleteDocument: deleteDocument, restoreDocument: restoreDocument,
        queryDocuments: queryDocuments, getAllDocuments: getAllDocuments, countDocuments: countDocuments,
        startBatch: startBatch, batchCreate: batchCreate, batchUpdate: batchUpdate, batchDelete: batchDelete,
        commitBatch: commitBatch, cancelBatch: cancelBatch,
        onDocumentSnapshot: onDocumentSnapshot, onQuerySnapshot: onQuerySnapshot, removeAllListeners: removeAllListeners,
        processOfflineQueue: processOfflineQueue, getOfflineQueueStatus: getOfflineQueueStatus, clearOfflineQueue: clearOfflineQueue,
        invalidateCache: invalidateCache, clearAllCache: clearAllCache,
        generateId: generateId, timestamp: timestamp, increment: increment,
        arrayUnion: arrayUnion, arrayRemove: arrayRemove, deleteField: deleteField,
        runTransaction: runTransaction,
        MAX_BATCH_SIZE: MAX_BATCH_SIZE, DEFAULT_PAGE_SIZE: DEFAULT_PAGE_SIZE,
    };
})();

window.CRM_Firestore = CRM_Firestore;
if (typeof module !== 'undefined' && module.exports) module.exports = CRM_Firestore;
